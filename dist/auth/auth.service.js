...
let AuthService = class AuthService {
    ...
    async register(userData) {
        if (!userData.employeeNumber || !userData.password) {
            throw new common_1.BadRequestException('Employee number and password are required');
        }
        const existingUser = await this.userRepository.findOne({
            where: { employeeNumber: userData.employeeNumber },
        });
        if (existingUser) {
            throw new common_1.UnauthorizedException('User with this employee number already exists');
        }
    ...
    async revokeRefreshToken(refreshToken) {
        await this.refreshTokenRepository.update({ token: refreshToken }, { isActive: false });
    }
